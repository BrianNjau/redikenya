...
const HeroCarousel = () => {
  let combinedSummaryData = () => {
    let numberSum = Object.values(summaryData).reduce(
      (acc, curr) => (acc = acc + curr["number"]),
      0
    );
    let unitSum = Object.values(summaryData).reduce(
      (acc, curr) => (acc = acc + curr["units"]),
      0
    );
    let accerageSum = Object.values(summaryData).reduce(
      (acc, curr) => (acc = acc + curr["accerage"]),
      0
    );
    let priceSum = Object.values(summaryData).reduce(
      (acc, curr) => (acc = acc + curr["price"]),
      0
    );
  ...
  let [calculatedSummaryData, setCalculatedSummaryData] =
    useState(combinedSummaryData);
  ...
  let handleSelect = (option) => {
    ...
    } else if (option.target.value === "Kileleshwa") {
      setCalculatedSummaryData(summaryData.kileleshwa);
    } else if (option.target.value === "Westlands") {
      setCalculatedSummaryData(summaryData.westands);
    } else if (option.target.value === "Lavington") {
      setCalculatedSummaryData(summaryData.lavington);
    } else if (option.target.value === "Riverside") {
  ...
  const IconWithTextData_04 = [
    ...
    },
    {
      ...
      title:
        calculatedSummaryData.price.toString().length > 9
          ? (calculatedSummaryData.price / 10 ** 9).toFixed(2).toString() + "B"
          : calculatedSummaryData.price.toString().length > 6
          ? (calculatedSummaryData.price / 10 ** 6).toFixed(2).toString() + "M"
          : calculatedSummaryData.price,
    ...
    >
          ...
          <Row className="h-[100vh] md:h-full">
            <Col className="flex flex-col">
              <div className="flex flex-row items-center  mb-0 mt-[20vh]">
                <span className="font-serif font-semibold text-[#3eb489] xs:text-xmd">
                  Our Data Inventory
                </span>
              ...
              <h1 className="font-serif font-semibold text-white text-[65px] leading-[80px] w-[70%] mt-[2vh] tracking-[-5px] lg:text-[55px] lg:leading-[100px] md:text-[45px] md:leading-[85px] sm:text-[25px] sm:leading-[50px] sm:-tracking-[.5px] xs:w-[60%] xs:text-[60px] xs:leading-[65px]">
                ...
                <span className="mr-[18px]">
                  {" "}
                  the quality of your real estate investment decisions
                </span>
              </h1>
              <div className="relative subscribe-style-05">
                <HeroSelect
                  selectedOption={selectedOption}
                  calculatedSummaryData={calculatedSummaryData}
      ...
      <SwiperSlide className="full-screen md:h-[600px] sm:h-[350px] md:flex md:items-center overflow-hidden relative bg-white">
        <Parallax
          ...
          translateY={[-40, 40]}
          style={{
            backgroundImage: `url(https://images.unsplash.com/photo-1596005554384-d293674c91d7?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=2698&q=80`,
          }}
        ></Parallax>
        <div
          className="absolute top-0 left-0 h-full w-full opacity-80
